...
int CardGame::askNumberOfCards() {
    ...
    while (true) {
        ...
        ventilator.send("How many Cards?", 15);

        zmq::message_t numCardsMsg;
        subscriber.recv(numCardsMsg, zmq::recv_flags::none);

        int num = *static_cast<int*>(numCardsMsg.data());

        if (num > 0) {
            return num;
        } else {
            ventilator.send("Positive number!", 16);
        }
