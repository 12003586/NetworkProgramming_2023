    ...
    try {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (input == "cards" || input == "Cards") {
                ...
                int numberOfHands = std:: stoi(input3);

                std::string input4 = "Gokspel<cards<Hands<" + input3;
                sendMessage(outgoingSocket, input4);

                std::cout << receiveMessage(incomingSocket) << std::endl;

                for(int i; i = numberOfHands;i++)
                {
                    std::getline(std::cin, input);
                    int numberOfCards = std:: stoi(input);

                    std::string input2 = "Gokspel<cards<Hands<" + std::to_string(i) + input;
                    sendMessage(outgoingSocket, input2);
            ...
            if (input == "tarot" || input == "Tarot")
            {
                std::cout << receiveMessage(incomingSocket) << std::endl;
                std::cout<< "tarot kaart" <<std::endl;
